...
function FloatingButton({ children, triggerContent }: FloatingButtonProps) {
  ...
  return (
    <div className="relative flex flex-col items-center">
      <AnimatePresence>
        <motion.ul key="list" className="absolute bottom-14 flex flex-col items-center gap-2" initial="hidden" animate={isOpen ? "visible" : "hidden"} variants={list}>
          {children}
        </motion.ul>
        <motion.div key="button" variants={btn} animate={isOpen ? "visible" : "hidden"} ref={ref} onClick={() => setIsOpen(!isOpen)} className="cursor-pointer">
          {triggerContent}
        </motion.div>
      </AnimatePresence>
    </div>
